...
class ErrorTranslator {
  ...
  static String translate(dynamic exception, {BuildContext? context}) {
    ...
    if (exception is ArgumentError) {
      return context?.l10n.errInvalidInput ??
  ...
  static String _handleFirebaseAuthError(
    ...
    switch (e.code) {
      case 'user-not-found':
        return context?.l10n.errAuthUserNotFound ??
            'Bu e-posta adresine ait bir kullanıcı bulunamadı.';
      case 'wrong-password':
        return context?.l10n.errAuthWrongPassword ??
            'Hatalı şifre girdiniz. Lütfen tekrar deneyin.';
      case 'email-already-in-use':
        return context?.l10n.errAuthEmailInUse ??
            'Bu e-posta adresi zaten kullanımda.';
      case 'invalid-email':
        return context?.l10n.errAuthInvalidEmail ??
            'Geçersiz bir e-posta adresi girdiniz.';
      case 'user-disabled':
        return context?.l10n.errAuthUserDisabled ??
            'Bu kullanıcı hesabı askıya alınmış.';
      case 'operation-not-allowed':
        return context?.l10n.errAuthNotAllowed ??
            'Bu giriş yöntemine şu an izin verilmiyor.';
      case 'weak-password':
        return context?.l10n.errAuthWeakPassword ??
  ...
  static String _handleFirebaseError(
    ...
    switch (e.code) {
      case 'permission-denied':
        return context?.l10n.errPermissionDenied ??
            'Bu veriye erişmek veya değiştirmek için yetkiniz bulunmuyor.';
      case 'unavailable':
        return context?.l10n.errDbUnavailable ??
            'Şu anda sunucuya ulaşılamıyor (Çevrimdışı olabilirsiniz).';
      case 'not-found':
        return context?.l10n.errDbNotFound ??
            'Erişmeye çalıştığınız veri bulunamadı.';
      case 'already-exists':
        return context?.l10n.errDbAlreadyExists ?? 'Bu kayıt zaten mevcut.';
      case 'resource-exhausted':
        return context?.l10n.errDbResourceExhausted ??
      ...
      case 'deadline-exceeded':
        return context?.l10n.errDbDeadlineExceeded ??
