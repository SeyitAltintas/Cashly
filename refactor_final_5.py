...
for i in range(media_start, len(lines)):
    if '// ─── Build' in lines[i] or '// --- Build' in lines[i]:
        media_end = i
        break
...
voice_end = -1
for i, line in enumerate(lines):
    if '// ─── Sesli Dikte' in line or '// --- Sesli Dikte' in line:
        voice_start = i
        break

for i in range(voice_start, len(lines)):
    if 'PreferredSizeWidget _buildAppBar(ColorScheme colorScheme) {' in lines[i]:
        voice_end = i
        break

print(f"Media block: {media_start} to {media_end}")
print(f"Voice block: {voice_start} to {voice_end}")

# Now build the new lines
for i, line in enumerate(lines):
    if media_start != -1 and media_end != -1 and media_start <= i < media_end:
        continue
    if voice_start != -1 and voice_end != -1 and voice_start <= i < voice_end:
        continue
    ...
    if 'bool _isRestarting = false;' in line:
        continue
    if "String _interimText = '';" in line and 'Son partial metin' in line:
        continue
    if "int _interimOffset = -1;" in line and 'Quill offset' in line:
        continue
    if "Timer? _voicePauseTimer;" in line:
        continue
    if "Timer?" in line and "_voiceSilenceTimer;" in lines[i + 1] if i + 1 < len(lines) else False:
        continue
    if "_voiceSilenceTimer;" in line and "Timer?" in lines[i - 1] if i - 1 >= 0 else False:
        continue
